...
// Only use #pragma comment with MSVC, not MinGW
#ifdef _MSC_VER
...
// REMOVE THIS LINE: #endif // _WIN32
// This was causing the "error: ‘#endif’ without ‘#if’"
...
// Swap information (Pagefile in Windows)
struct Swap {
    uint64_t total_bytes = 0;
    uint64_t used_bytes = 0;
    uint64_t free_bytes = 0;
...
#ifdef _WIN32
// WMI helper class (Windows only)
class WMIQuery {
public:
    WMIQuery();
    ~WMIQuery();
    
    bool executeQuery(const std::string& query, std::vector<std::vector<std::pair<std::string, std::string>>>& results);
    std::vector<std::string> getPropertyValues(const std::string& query, const std::string& property);
...
private:
    ...
    std::vector<std::string> getWmiProperty(const std::string& query, const std::string& property);
    uint64_t getFileTimeToSeconds(FILETIME ft);
